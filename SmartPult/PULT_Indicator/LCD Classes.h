...
public:
    ...
    virtual void SetText(char* ptext) {p_text = ptext;} //сохраняет текст для себя
    ...
    virtual void ReDraw(){Drawed = false; Draw();} //рисует ячейку в стиле Active_Style
    virtual void ReHide(){  Hided = false; Hide();} //рисует ячейку в стиле UnActive_Style
    void Clean(){PreDraw();} //очищает ячейку (рисует пустую)
    virtual void Listener(); //будет наследоваться всеми экранными формами для анализа изменения кнопок и данных
    void DrawTecStile();
